...
export interface FaultInjection {
  // Network faults
  dropPacketRate?: number;        // 0-1, probability of dropping incoming packets
  responseDelay?: number;         // ms delay before responding
  networkTimeout?: boolean;       // Simulate complete network failure

  // sACN faults
  sacnSignalLoss?: boolean;       // Stop receiving sACN
  sacnPacketErrors?: number;      // Rate of packet errors

  // Device faults
  randomReboot?: number;          // Probability of random reboot per request
  heapExhaustion?: boolean;       // Simulate low memory
  watchdogTrigger?: boolean;      // Simulate watchdog reboot
}

export interface SimulatorOptions {
  basePort?: number;              // Starting port for devices (default 8080)
  logLevel?: 'debug' | 'info' | 'warn' | 'error';
  enableMdns?: boolean;           // Enable mDNS advertisement (default true)
  sacnSimulation?: boolean;       // Simulate sACN reception (default true)
}

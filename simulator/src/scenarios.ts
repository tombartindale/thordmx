import type { TestScenario, SimulatedDeviceConfig, FaultInjection } from './types.js';
...
export const scenarios: Record<string, TestScenario> = {
  ...
  'single-device': {
    ...
    deviceConfigs: [
      { name: 'DMX-Bridge-001', universe: 1 },
    ],
  },
  ...
  'low-memory': {
    ...
    deviceConfigs: [
      { name: 'Memory-Leak-Device', universe: 1 },
    ],
    faultInjection: {
      heapExhaustion: true,
    },
 ...
 */
export function listScenarios(): Array<{ id: string; name: string; description: string; deviceCount: number }> {
